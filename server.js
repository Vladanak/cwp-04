...
const server = net.createServer((client) =>
{
    ...
    client.on('data', (data) =>
    {
        ...
        if
        (clients[client.id] === reqQA && data !== reqQA)
        {
            console.log(data);
            let questionInd = getQuestionId(data);
            let answer;
            if (answerTheQuestion() === 1)  { answer = arr[questionInd].goodAns;}
            else { answer = arr[questionInd].badAns; }
            client.write(answer);
        }
        ...
        if (clients[client.id] === reqRemote && data !== reqRemote)
        {
            ...
            if (remoting[0] === reqRemCopy) { rd.pipe(wr).on('close', sendNext); }
    ...
    function sendNext()  { client.write(resGood); }
    client.on('end', () =>
    {
        connections--;
...
//*************************************
function getUniqID()  { return Date.now() + seed++; }
...
function answerTheQuestion() { return Math.random() > 0.5 ? 1 : 0; }