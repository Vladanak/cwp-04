...
client.connect(port, () => { client.write(req); });

function getDirs() { process.argv.slice(2).forEach((dir) => { readFiles(dir); }); }
...
client.on('data', (data) =>
{
    ...
    if (arrOfFiles.length === 0)
        client.destroy();
    else if (data === badResp) { client.destroy(); }
    else if (data === goodResp || data === resFiles) { sendFile() }
